...
export default function FloorEditor() {
  ...
  const [tool, setTool] = useState<SymType | 'select'>('select');
  ...
  const undo = () => { const p = past.current.pop(); if (p) { setItems(p); setSelected(null); } };
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') { if (full) setFull(false); else { setTool('select'); setSelected(null); setDraft(null); } }
      else if ((e.key === 'Delete' || e.key === 'Backspace') && selected) { e.preventDefault(); commit(items.filter((i) => i.id !== selected)); setSelected(null); }
      else if ((e.key === 'r' || e.key === 'R') && sel) commit(items.map((i) => i.id === sel.id ? { ...i, rot: (i.rot + 90) % 360 } : i));
  ...
  const renumber = () => { let n = 1; const sorted = [...items].filter((i) => i.type === 'desk').sort((a, b) => (Math.round(a.y / 40) - Math.round(b.y / 40)) || (a.x - b.x)); const map = new Map(sorted.map((d) => [d.id, `${prefix} ${pad(n++)}`.trim()])); seq.current = n; commit(items.map((i) => map.has(i.id) ? { ...i, label: map.get(i.id) } : i)); };
  ...
  const onMouseDown = (e: Konva.KonvaEventObject<MouseEvent>) => {
    ...
    if (tool === 'select') { if (isEmpty) setSelected(null); return; }
    const c = coords(); if (!c) return;
    if (DRAG_TYPES.has(tool)) { const x = snap(c.x, snapOn), y = snap(c.y, snapOn); setDraft({ type: tool, x1: x, y1: y, x2: x, y2: y }); }
    else place(tool, snap(c.x, snapOn), snap(c.y, snapOn));
  };
  const onMouseMove = () => { if (pan.current) { const p = stageRef.current?.getPointerPosition(); if (p) setPos({ x: pan.current.ox + (p.x - pan.current.sx), y: pan.current.oy + (p.y - pan.current.sy) }); return; } if (!draft) return; const c = coords(); if (!c) return; setDraft({ ...draft, x2: snap(c.x, snapOn), y2: snap(c.y, snapOn) }); };
  const onMouseUp = () => {
    if (pan.current) { pan.current = null; return; }
    if (!draft) return;
    const { type, x1, y1, x2, y2 } = draft; const dx = x2 - x1, dy = y2 - y1;
    if (type === 'deskGrid') { const gx = Math.min(x1, x2), gy = Math.min(y1, y2); if (Math.abs(dx) > GRID && Math.abs(dy) > GRID) commit([...items, ...makeDeskGrid(gx, gy, Math.abs(dx), Math.abs(dy))]); setTool('select'); setSelected(null); }
    else if (type === 'wall') {
      ...
      commit([...items, item]); setTool('select'); setSelected(id);
    } else {
  ...
  const hint = tool === 'select' ? 'Click to edit · drag to move · right-drag to pan'
    : tool === 'deskGrid' ? 'Drag a rectangle → fills with numbered desks'
    : tool === 'bay' ? 'Drag to create a named, colour-coded bay'
    : DRAG_TYPES.has(tool) ? `Drag to draw ${tool}` : `Click once to place ${tool}`;

  const renderItem = (it: EItem) => {
    const isSel = it.id === selected; const hw = Math.max(it.w, 22), hh = Math.max(it.h, 22);
    return (
      <Group key={it.id} x={it.x + it.w / 2} y={it.y + it.h / 2} offsetX={it.w / 2} offsetY={it.h / 2} rotation={it.rot}
        draggable={tool === 'select'}
        onClick={() => { if (tool === 'select') setSelected(it.id); }} onTap={() => { if (tool === 'select') setSelected(it.id); }}
        onDragEnd={(e) => { const nx = snap(e.target.x() - it.w / 2, snapOn), ny = snap(e.target.y() - it.h / 2, snapOn); commit(items.map((i) => i.id === it.id ? { ...i, x: nx, y: ny } : i)); }}
      >
        ...
        <Glyph it={it} />
        {it.type === 'desk' && it.label && scale > 0.85 && <Text y={it.h / 2 - 4} width={it.w} align="center" text={it.label.replace(/^\D+/, '')} fontSize={8} fill="#33415588" listening={false} />}
        {isSel && <Rect x={-4} y={-4} width={it.w + 8} height={it.h + 8} stroke="#064281" strokeWidth={1.5} dash={[5, 4]} cornerRadius={4} listening={false} />}
      </Group>
    );
  };
  ...
  return (
    <div className={'fed' + (full ? ' fed--full' : '')}>
      <div className="fed-palette">
        <button className={'fed-tool' + (tool === 'select' ? ' on' : '')} style={{ width: '100%', marginBottom: 12 }} onClick={() => setTool('select')}><Icon name="crosshair" size={15} /> Select / Move</button>
        {GROUPS.map((g) => (
          <div className="fed-group" key={g.title}>
            <div className="fed-group__t">{g.title}</div>
            <div className="fed-syms">
              {g.items.map((s) => (
                <button key={s.type} className={'fed-sym' + (tool === s.type ? ' on' : '')} title={s.label} onClick={() => { setTool(s.type); setSelected(null); }}>
      ...
      <div className="fed-stagewrap" ref={wrapRef}>
        <div className="fed-toolbar">
          ...
          <span className="fed-numctl"><span>Desk #</span><input className="input fed-num-prefix" value={prefix} onChange={(e) => setPrefix(e.target.value)} title="Desk number prefix" /><button className="btn btn--ghost btn--sm" onClick={renumber}>Renumber</button></span>
        ...
        <Stage
          ...
          style={{ background: '#e9eef5', cursor: tool === 'select' ? 'default' : 'crosshair' }}
        >
          <Layer>
        ...
        {sel ? (
          <>
            ...
            <div className="fed-actions">
              ...
              <button className="btn btn--ghost btn--sm is-danger" onClick={() => { commit(items.filter((i) => i.id !== sel.id)); setSelected(null); }}><Icon name="trash" size={14} /> Delete</button>
            </div>
          ...
          <>
            ...
            <ol className="fed-help">
              ...
              <li>Other symbols: pick one, <b>click once</b> to place (it then switches to Select).</li>
              <li><b>Right-mouse drag</b> = pan · wheel = zoom · <b>Full screen</b> for big floors.</li>
              <li>Meeting rooms &amp; tables: set <b>PAX</b> in the panel — chairs update automatically.</li>
            </ol>
