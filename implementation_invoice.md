...
- _2026-06-04_ — Summary look-and-feel: replaced the flat grey summary bar with a reusable **`StatCards`** strip (icon chip + large value + label, hover lift, responsive auto-fit grid; matches the Overview KPI quality) on the **Billing Batches** list (Bill files / Billing months / Invoices / Pending approval) and the **batch detail** summary (Invoices / Basic / GST / **Total — accented** / Entities / Billing Month). The role-aware bulk-action toolbar keeps its bar styling (it's an action bar, not a summary). New `.stat-strip`/`.stat-card` styles + `Stat`/`StatCards` in invoicing ui. Type-check clean; pages verified 200.
- _2026-06-04_ — Summary refinements: **Billing Batches** summary is now a **status breakdown** (one stat card per present batch status, count + status colour/icon, in workflow order, filter-aware) instead of generic counts; **batch detail** summary drops the **Entities (per invoice)** card (Invoices / Basic / GST / Total / Billing Month). Type-check clean; verified 200.
- _2026-06-04_ — Sub-nav (tab bar) UX overhaul: **sticky** under the 64px topbar so it stays in reach on long pages; active tab is now a **filled brand-blue gradient pill** (white icon+label, soft shadow) instead of the flat underline; smoother tinted hover with icon brighten; and **attention badges** on the tabs surfacing live counts — Billing Batches (pending approvals), Reconciliation (open reconciliations), Notifications (paid-but-unsent) — colour-flipped on the active pill. Type-check clean; verified 200 (badges 1/2/3 render).
- _2026-06-04_ — Configuration CRUD: **Paying entities** now support **Edit + Delete** (icon actions) alongside Add — Add/Edit share one inline form (pre-filled on edit), delete with confirm; **Expense categories** gain **inline rename (Edit) + Delete** in an editable list, plus Add. All mutate local state (becomes server-backed in the backend phase). invoice.md §13.9 synced. Type-check clean; verified 200.
- _2026-06-04_ — Vendor notification email enhanced to a proper **payment advice** (replaces the plain alert): a clean branded email preview (modal) that mirrors the **finance report** for that vendor — summary (payment date, mode, single UTR, net) + a **Payment Details** table with per-invoice **Dr** (Basic+GST split, gross) and **Cr** `-TDS` lines, then **Gross total → less TDS → Net paid**. Columns: Payment Date · Particulars · Invoice Ref No · Amount · Dr/Cr · Total Amount Paid · UTR No (single combined **Amount** per line, net shown as **Total Amount Paid** — two amount columns mirror the finance report; Particulars reads "Agst Ref"). Per-vendor **Preview** (eye) action + bottom Preview; new `.email*` styles + `fmtDate` helper. invoice.md §13.6 synced. Type-check clean; verified 200.
